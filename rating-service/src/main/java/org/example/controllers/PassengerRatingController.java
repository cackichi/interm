...
@RestController
@RequestMapping("/api/rating/pass")
@AllArgsConstructor
public class PassengerRatingController {
    ...
    public String findRating(@PathVariable("id") Long id){
        try {
            return "Рейтинг пассажира с id" + ":" + id + " = " + passengerRatingService.findRating(id);
    ...
    public List<PassengerRating> findAll(){
        return passengerRatingService.findAllNotDeleted();
    }

    @PostMapping("/create")
    public ResponseEntity<String> create(@RequestBody PassengerRatingDTO passengerRatingDTO){
