...
public interface RideRepository extends JpaRepository<Ride, Long> {
    @Modifying
    @Query("UPDATE Ride r SET r.deleted = true WHERE r.passengerId = :id")
    void softDelete(@Param("id") Long id);

    @Modifying
    @Query("UPDATE Ride r SET r.pointA = :pointA, r.pointB = :pointB WHERE r.passengerId = :id")
    ...
    @Modifying
    @Query("UPDATE Ride r SET r.status = :status WHERE r.passengerId = :id")
